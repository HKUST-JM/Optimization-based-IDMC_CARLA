...
if __name__ == '__main__':
    ...
    ego_vehicle.solver_basis(Q=np.diag([3, 2, 0.5, 1.5, 0]))
    ...
    while simu_time < time_max:
        ...
        ## switch on the traffic light at 10s
        if z0[0]>(tr_lgts-10) and simu_time < 30:
            print("Red Light!!!", simu_time)
            ego_vehicle.solver_add_single_tr_lgt_pf(tr_lgts)
            l_state = True
        else:
            l_state = False

        z_init = [[ego_vehicle.x]*T, [ego_vehicle.y]*T,
                      [ego_vehicle.yaw]*T, [ego_vehicle.vx]*T, [ego_vehicle.vy]*T, [0]*T]

        # if l_state == True:
        #     z_ref = np.array(z_init).T
        # else:
        z_ref, target_ind = \
        ...
        obs_2 = np.array(
            [[ref_path.cx[cnt+40], ref_path.cy[cnt+80]-5, ref_path.cyaw[cnt+15]]])
        obs_2_path.append(obs[0])

        
        # ego_vehicle.for_comparison_solver_add_soft_obs(obs)
        # ego_vehicle.for_comparison_solver_add_soft_obs(obs_2)

        ego_vehicle.solver_add_soft_obs(obs)
        ego_vehicle.solver_add_soft_obs(obs_2)
        ...
        ego_vehicle.solver_add_cost(tr_lgt=l_state)
        ego_vehicle.solver_add_bounds(tr_lgt=l_state)
        ...
        for i in range(obs.shape[0]):
            ...
            draw.draw_car(x=obs_2[i, 0], y=obs_2[i, 1],
                          yaw=obs_2[i, 2], steer=0, color='red')
        ...
        temp_path = np.array(obs_2_path)
        plt.plot(temp_path[:, 0], temp_path[:, 1], color='yellow', alpha=0.5)
        plt.plot(x, y, '-b')
        # plt.plot(cx[target_ind], cy[target_ind])
        if l_state == False:
            tr_lgt = plt.Circle((tr_lgts, 5), 1, color='g')
        else:
            tr_lgt = plt.Circle((tr_lgts, 5), 1, color='r')
        plt.gca().add_patch(tr_lgt)
