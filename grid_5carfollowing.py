from scripts.vehicle_following import Vehicle
...
if __name__ == '__main__':
    ...
    # ay = [0.0, 35.0, 40.0, 30.0, 0.0]

    obs = np.array([[15.0, 30.0, 1.57*0.5], [61, 30, -1.57/2]])
    ...
    # ref_path = PATH(cx[1:], cy[1:], cyaw[1:], ck[1:])
    ref_path = PATH(cx, cy, cyaw, ck)
    ego_vehicle = Vehicle((cx[0], cy[0], cyaw[0], 0.0),
                          horizon=T, target_v=18, follow_dis=10, carla=False)

    ref_size = len(ref_path.cx)
    ...
    cnt = 200
    ego_vehicle.solver_basis(Q=np.diag([3, 3, 0, 0, 0]))
    ...
    while simu_time < time_max:
        ...
        z_ref = np.array(
            [ref_path.cx[cnt:cnt+T], ref_path.cy[cnt:cnt+T], [1]*T, ref_path.cyaw[cnt:cnt+T]])

        z_ref = np.array([z_ref[0, :], z_ref[1, :], z_ref[3, :], z_ref[2, :], [
                         0]*z_ref.shape[1], [0]*z_ref.shape[1]]).T
        print("ref "+str(z_ref[0, :]))
        '''
